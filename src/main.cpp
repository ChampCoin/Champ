...
#if defined(NDEBUG)
# error "Champcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x768706d295c5d827b188ae9cae2124555e624c6bed7c5f7d9443706272b309b5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Champcoin: starting difficulty is 1 / 2^12
...
{
    // Champcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 335 * COIN;
	
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 60; // Champcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 60; // Champcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Champcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Champcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x768706d295c5d827b188ae9cae2124555e624c6bed7c5f7d9443706272b309b5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CNN Money 28/Dec/2015 Oil prices fall again, down 11% in month";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1451303365;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1265887;
        ...
        assert(block.hashMerkleRoot == uint256("0x91af7ff613fae9f65d9f33315cc71df0fd0225bfe6773a7269aa20980b9c4fd4"));
...
unsigned char pchMessageStart[4] = { 0xc5, 0xd4, 0xd3, 0xbd }; // Champcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ChampcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChampcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ChampcoinMiner(CWallet *pwallet)
{
    printf("ChampcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("champcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ChampcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ChampcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ChampcoinMiner, pwallet));
}
